...
module.exports = function(element) {
  ...
  switch (element[0].name) {
    ...
    case this.components.row:
      ...
      return format('<table %s class="%s"><tr style="font-size: 0; text-align: center;">%s</tr></table>', attrs, classes.join(' '), inner);
    ...
    case this.components.container:
      ...
      return format('<table %s align="center" class="%s"><tr><td class="container__inner">%s</td></tr></table>', attrs, classes.join(' '), inner);
    ...
    case this.components.spacer:
      ...
      if (element.attr('size-sm') || element.attr('size-lg')) {
        if (element.attr('size-sm')) {
          ...
          html += format('<table %s class="%s hide-for-large"><tr><td height="'+size+'px" style="font-size:'+size+'px;line-height:'+size+'px;">&nbsp;</td></tr></table>', attrs);
        }
        if (element.attr('size-lg')) {
          size = (element.attr('size-lg'));
          html += format('<table %s class="%s show-for-large"><tr><td height="'+size+'px" style="font-size:'+size+'px;line-height:'+size+'px;">&nbsp;</td></tr></table>', attrs);
        }
      } else {
        size = (element.attr('size')) || 16;
        html += format('<table %s class="%s"><tr><td height="'+size+'px" style="font-size:'+size+'px;line-height:'+size+'px;">&nbsp;</td></tr></table>', attrs);
      }
    ...
    case this.components.container:
      ...
      return format('<table %s align="center" class="%s"><tr><td class="new">%s</td></tr></table>', attrs, classes.join(' '), inner);
